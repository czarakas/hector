 ...
 *
 *  Structure of the ocean component (high latitude, low latitude, intermediate
 & deep ocean) is
 ...
 *  Hartin, C. A., Bond-Lamberty, B., Patel, P., and Mundra, A.: Ocean
 acidification over
 *      the next three centuries using a simple global climate carbon-cycle
 model:
...
OceanComponent::OceanComponent() { spinup_chem = true; }
...
unitval OceanComponent::sendMessage(const std::string &message,
  ...
  if (message == M_GETDATA) { //! Caller is requesting data
    ...
    H_LOG(logger, Logger::DEBUG) << "Atmosphere dumping " << carbon
                                 << " Pg C to deep ocean" << std::endl;
...
//------------------------------------------------------------------------------
// documentation is inherited
void OceanComponent::setData(const string &varName, const message_data &data) {
...
void OceanComponent::prepareToRun() {
  ...
  // Advection --> transport of carbon from one box to the next (k values,
  // fraction/yr )
...
fluxpool OceanComponent::totalcpool() const {
  return deep.get_carbon() + inter.get_carbon() + surfaceLL.get_carbon() +
         surfaceHL.get_carbon();
}

//------------------------------------------------------------------------------
/*! \brief                  Internal function to calculate atmosphere-ocean C
 * flux \param[in] date         double, date of calculation (in case constraint
 * used) \param[in] CO2_conc     unitval, atmospheric CO2 concentration
 *  \param[in] cpoolscale   double, how much to scale surface C pools by
 *  \returns                unitval, annual atmosphere-ocean C flux
 */
unitval OceanComponent::annual_totalcflux(const double date,
                                          const unitval &CO2_conc,
                                          const double cpoolscale) const {
...
void OceanComponent::run(const double runToDate) {
  ...
  H_LOG(logger, Logger::DEBUG)
      << "Starting new year: SST= " << SST << std::endl;
...
//------------------------------------------------------------------------------
// documentation is inherited
unitval OceanComponent::getData(const std::string &varName, const double date) {
  ...
  if (date == Core::undefinedIndex()) {
    ...
    if (varName == D_OCEAN_C_UPTAKE) {
      ...
      double value =
          part_low *
              surfaceLL.mychemistry.convertToDIC(surfaceLL.get_carbon()) +
          part_high *
              surfaceHL.mychemistry.convertToDIC(surfaceHL.get_carbon());
      ...
      double value = part_low * surfaceLL.mychemistry.PCO2o +
                     part_high * surfaceHL.mychemistry.PCO2o;
      ...
      double value = part_low * surfaceLL.mychemistry.pH +
                     part_high * surfaceHL.mychemistry.pH;
      ...
      double value = part_low * surfaceLL.mychemistry.CO3 +
                     part_high * surfaceHL.mychemistry.CO3;
      returnval = unitval(value, U_UMOL_KG);
    ...
    if (varName == D_OCEAN_C_UPTAKE) {
      ...
      returnval = C_DO_ts.get(date) + C_IO_ts.get(date) + Ca_LL_ts.get(date) +
                  Ca_HL_ts.get(date);
    } else if (varName == D_HL_DO) {
      ...
      double value =
          part_low * PH_LL_ts.get(date) + part_high * PH_HL_ts.get(date);
      returnval = unitval(value, U_PH);
    } else if (varName == D_ATM_OCEAN_FLUX_HL) {
      ...
      double value =
          part_low * pco2_LL_ts.get(date) + part_high * pco2_HL_ts.get(date);
      returnval = unitval(value, U_UATM);
    } else if (varName == D_DIC_HL) {
      ...
      double value =
          part_low * dic_LL_ts.get(date) + part_high * dic_HL_ts.get(date);
      returnval = unitval(value, U_UMOL_KG);
    } else if (varName == D_CARBON_HL) {
      ...
      double value =
          part_high * co3_HL_ts.get(date) + part_low * co3_LL_ts.get(date);
      returnval = unitval(value, U_UMOL_KG);
    } else {
    ...
    H_ASSERT(date == Core::undefinedIndex(),
             "Date data not available for " + varName +
                 " in OceanComponent::getData()");
  }
...
int OceanComponent::calcderivs(double t, const double c[],
  ...
  dcdt[SNBOX_OCEAN] =
      annual_totalcflux(t, CO2_conc, cpoolscale).value(U_PGC_YR);

  // If too big a timestep--i.e., stashCvalues below has signalled a reduced
  // step that we're exceeding--signal to the solver that this won't work for
  // us.
...
void OceanComponent::stashCValues(double t, const double c[]) {
  ...
  H_LOG(logger, Logger::DEBUG)
      << "Stashing at t=" << t << ", model pools at " << t << ": " << c[0]
      << " " << c[1] << " " << c[2] << " " << c[3] << " " << c[4] << " " << c[5]
      << std::endl;
  ...
  const double yearfraction = (t - ODEstartdate);
  // H_LOG( logger, Logger::NOTICE ) << "Solver has finished. Yearfraction = "
  // << yearfraction << std::endl;
  H_ASSERT(yearfraction >= 0 && yearfraction <= 1,
           "yearfraction out of bounds");
  ...
  // end-of-timestep values, and we need to overwrite them with what the solver
  // has sent us (~mid-timestep values), so that everything stays consistent.
  ...
  if (currentflux.value(U_PGC))
    adjustment = (solver_flux - currentflux) / 2.0;
  H_LOG(logger, Logger::DEBUG)
      << "Solver flux = " << solver_flux << ", currentflux = " << currentflux
      << ", adjust = " << adjustment << std::endl;
  ...
  // Separate the one net flux (can be positive or negative) into the two
  // fluxpool fluxes (always positive) This updates oa_flux and ao_flux within
  // the two ocean boxes
  ...
  if (cflux_annualdiff.value(U_PGC) > OCEAN_TSR_TRIGGER1) {
    ...
    H_LOG(logger, Logger::DEBUG)
        << "Reducing timestep to " << max_timestep << ": t=" << t
        << " yearfraction=" << yearfraction << std::endl;
    H_LOG(logger, Logger::DEBUG)
        << " solver_flux=" << solver_flux
        << " lastflux_annualized=" << lastflux_annualized;
    H_LOG(logger, Logger::DEBUG)
        << " cflux_annualdiff=" << cflux_annualdiff << std::endl;
    ...
    H_LOG(logger, Logger::DEBUG)
        << "OK, reduced_timestep_timeout =" << reduced_timestep_timeout
        << std::endl;
    if (!reduced_timestep_timeout) {
      H_LOG(logger, Logger::DEBUG)
          << "Reduced ts timeout done; raising" << std::endl;
      max_timestep = min(OCEAN_MAX_TIMESTEP, max_timestep / OCEAN_TSR_FACTOR);
      if (max_timestep < OCEAN_MAX_TIMESTEP) {
        reduced_timestep_timeout =
            OCEAN_TSR_TIMEOUT; // set timer for another raise attempt
      }
    }
  }
  ...
  H_LOG(logger, Logger::DEBUG)
      << "lastflux_annualized=" << lastflux_annualized << std::endl;
  H_LOG(logger, Logger::DEBUG)
      << "annualflux_sum=" << annualflux_sum << std::endl;
...
void OceanComponent::record_state(double time) {
  H_LOG(logger, Logger::DEBUG)
      << "Recording component state at t= " << time << endl;
...
//------------------------------------------------------------------------------
// documentation is inherited
void OceanComponent::accept(AVisitor *visitor) { visitor->visit(this); }
